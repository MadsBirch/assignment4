...
- Loop 2: Run CRQA with median parameters
- Turn loop2 into a function
- Make shuffled files and use function on them
- Make surrogate files and use function on them
...
#loop2DF = data.frame()
loop2_shuf = data.frame()
loop2_test = data.frame()
#i = PP_final[1]

#------ Loop 2: CRQA ------
for (i in Shuffle_liezt){
  #------ Read file ------
  file = read.csv(i, header = TRUE)
  ...
  df_result = data.frame(Study, 
                             Pair_number, 
                             Condition, 
    "RR_h" = RR_h, "DET_h"=DET_h, "NRLINE_h"=NRLINE_h, "maxL_h"=maxL_h, "L_h"=L_h, "ENTR_h"=ENTR_h, "rENTR_h"=rENTR_h, "LAM_h"=LAM_h, "TT_h"=TT_h, 
    "RR_r"=RR_r, "DET_r"=DET_r, "NRLINE_r"=NRLINE_r, "maxL_r"=maxL_r, "L_r"=L_r, "ENTR_r"=ENTR_r, "rENTR_r"=rENTR_r, "LAM_r"=LAM_r, "TT_r"=TT_r
                             )
...
lapply(crqa_func, Shuffle_liezt)
...
s2_list = list.files('PP_data', pattern = "Study2", full.names = TRUE)
...
s1_list[grep(f1, s1_list)]

s1_list[grep(f1, s1_list)] 

s1_list[grep(co, s1_list)]

hest = s1_list[grep(f1, s1_list)grep(co, s1_list)]
...
for (co in c("Synchronous","TurnTaking","SelfPaced")){ #loop through a list of condintions 
  for (i in 1:nrow(SurrogateList)){ #and loop through all the pairings of the surrogate list
    ...
    d3 = data.frame(Resp1 = d1$Resp1, Resp2 = d2$Resp2, HR1 = d1$HR1, HR2 = d2$HR2, Resp1S = d1$Resp1, Resp2S = d2$Resp, HR1S = d1$HR1, HR2S = d2$HR2)
    ...
    #pair1person2 w pair2person2 1222, same as line 62??????
    ...
    #write csv files
    write_csv(d3,file.path("Surrogate_data",file3))
    write_csv(d4,file.path("Surrogate_data",file4))
    write_csv(d5,file.path("Surrogate_data",file5))
    write_csv(d6,file.path("Surrogate_data",file6))
  }
}
